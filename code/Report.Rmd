...
suppressWarnings(plotResiduals(hh_profit, hh_profit_model_topfeatures, "green", "TOP"))
```
...
Fit unrestricted model for urban area profit. We notice that the adjusted R square is really low, and there are a lot of correlated variable. By looking at data size, we see that there are less observations than variables for urban are profit data. So we don't have enough data to fit a model.
...
We also want to see how education and local characteristic info alone influence profit. Looking at hypothesis test result, loc5 and loc3 are correlated with other variables. So remove them and fit model again.
